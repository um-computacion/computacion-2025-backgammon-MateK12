...
from src.pygame_ui.Tablero_UI.Tablero_UI import (
    TableroUI,
    BOARD_WIDTH,
    BOARD_HEIGHT,
    BOARD_X,
...
class TestTableroUI(unittest.TestCase):
    ...
    @patch(
        "src.pygame_ui.Tablero_UI.Tablero_UI.TableroUI._TableroUI__dibujar_numero_triangulo"
    )
    @patch("src.pygame_ui.Tablero_UI.Tablero_UI.TableroUI.dibujar_triangulo")
    @patch("pygame.draw.rect")
    def test_dibujar_tablero(
        self, mock_draw_rect, mock_dibujar_triangulo, mock_dibujar_numero_triangulo
    ):
        screen = pygame.Surface((BOARD_WIDTH + 200, BOARD_HEIGHT + 100))
    ...
    @patch(
        "src.pygame_ui.Tablero_UI.Tablero_UI.TableroUI.get_punto_position_base_ficha"
    )
    def test_dibujar_fichas_en_punto(
        self, mock_get_punto_position_base_ficha, mock_dibujar_ficha
    ):
        screen = pygame.Surface((BOARD_WIDTH + 200, BOARD_HEIGHT + 100))
        punto_index = 0
    ...
    def test_dibujar_fichas_en_punto_vacio(
        self, mock_get_punto_position_base_ficha, mock_dibujar_ficha
    ):
        screen = pygame.Surface((BOARD_WIDTH + 200, BOARD_HEIGHT + 100))
        punto_index = 1
    ...
    def test_get_punto_position_base_ficha_coordenadas_correctas(self):
        ...
        for i in range(18, 24):
            x, y = self.tablero_UI.get_punto_position_base_ficha(i)
            expected_x = x_base + (i - 18) * punto_width + 6 * punto_width + 110
            expected_y = y_inferior
